        ...
        <ul class="nav navbar-nav navbar-right">
          ...
          <li class="active">
            ...
            <ul class="dropdown-menu">
			  ...
			  <li class="active"><a href="./list.php">礼金列表</a></li>
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>ID</th><th>ＱＱ</th><th>添加时间</th><th>是否还礼</th><th>操作</th></tr></thead>
          <tbody>
<?php
$rs=$DB->query("SELECT * FROM black_list WHERE{$sql} order by id desc limit $pageu,$pagesize");
while($res = $DB->fetch($rs))
{
echo '<tr><td>'.$res['id'].'</td><td>'.$res['qq'].'</td><td>'.$res['date'].'</td><td>'.$res['level'].'</td><td><a href="./edit.php?my=update&id='.$res['id'].'" class="btn btn-xs btn-info">修改</a> <a href="./edit.php?my=del&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确定要删除此记录吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
